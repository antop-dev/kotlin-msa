...
class WithKeyword {
    infix fun `json path`(expression: String): JsonPathResultMatchers = jsonPath("\$" + expression)
}

val With = WithKeyword()

class ThatKeyword {
    infix fun `status is http`(value: Int): ResultMatcher = status().`is`(value)
}

val That = ThatKeyword()

infix fun JsonPathResultMatchers.`that the value is`(value: Any): ResultMatcher = this.value(value)
infix fun ResultActions.`and expect`(matcher: ResultMatcher): ResultActions = this.andExpect(matcher)
infix fun ResultActions.`and then do`(handler: ResultHandler): ResultActions = this.andDo(handler)
infix fun MockMvc.`do a get request to`(uri: String): ResultActions = this.perform(get(uri))
